...
OSLRef.api = class {
	...
	static getPage(title) {
		...
		api.get({
			...
			for (var page_id of Object.keys(data.query.pages)) {
				...
				page = OSLRef.api._createPageObjectFromApiResult(page_data);
			}
			//console.log(page);
	...
	static getPages(titles) {
		...
		api.get({
			...
			for (var page_id of Object.keys(data.query.pages)) {
				var page_data = data.query.pages[page_id];
				pages.push(OSLRef.api._createPageObjectFromApiResult(page_data));
			}
			//console.log(page);
	...
	static getFilePage(name, dataType = "text") {
		const deferred = $.Deferred();
		OSLRef.api.getPage("File:" + name).then((page) => {
	...
	static editSlots(page, summary = "", mode = 'action-multislot') {
		if (mode === 'action-multislot') {
			...
			for (var slot_key of Object.keys(page.slots)) {
				...
				//OSLRef.api.editSlot(page.title, slot_key, page.slots[slot_key], summary); //parallel edit does not work
			}
			...
			function do_edit() {
				...
				if (slot_key) {
					...
					var content = page.slots[slot_key];
					if (page.content_model[slot_key] === 'json' && !(typeof content === 'string')) content = JSON.stringify(content);
					OSLRef.api.editSlot(page.title, slot_key, content, summary).done(do_edit);
				}
	...
	static copyPage(sourceTitle, targetTitle, summary = "", modify = undefined) { //(p) => { const d = $.Deferred(); d.resolve(p); return d.promise(); }) {
		...
		const deferred = $.Deferred();
		OSLRef.api.getPage(sourceTitle).then((sourcePage) => {
			OSLRef.api.getPage(targetTitle).then((targetPage) => {
				if (targetPage.exists) {
					OO.ui.confirm('Page does exist. Overwrite?').done((confirmed) => {
						if (confirmed) {
							if (summary === "") summary = "Copy of [[" + sourceTitle + "]]";
							OSLRef.api.copyPageContent(sourcePage, targetPage);
							targetPage.changed = true;
							modify(targetPage).then((targetPage) => {
								OSLRef.api.updatePage(targetPage, summary).then(() => {
	...
	static purgePage(title, options) {
		...
		var config = OSLRef.util.mergeDeep(defaultOptions, options);
	...
	static updatePage(page, meta) {
		...
		meta = meta || {comment: ""};
		if (OSLRef.util.isString(meta)) meta = {comment: meta}; //backwards compatibility
		...
		if (!page.exists && page.title && (page.content || page.slots['main'])) {
			OSLRef.api.createPage(page.title, page.content, summary).then((data) => {
				page.changed = false;
				page.exists = true;
				OSLRef.api.editSlots(page, summary).then((data) => { //will only edit changed slots
					if (hasChangedFile) {
						OSLRef.api.uploadFile(page.file.contentBlob, page.file.name, summary).then((data) => {
			...
			OSLRef.api.editSlots(page, summary).then((data) => {
				page.changed = false;
				page.exists = true;
				if (hasChangedFile) {
					OSLRef.api.uploadFile(page.file.contentBlob, page.file.name, summary).then((data) => {
	...
	static getLabels(titles, lang="en") {
		...
		for (const title of titles) {
			...
			if (title.startsWith("http://") || title.startsWith("https://")) {
				let uuid = OSLRef.util.uuidv4(title);
				query += "HasUuid::" + uuid;
			}
			else {
		...
		fetch(query)
			...
			.then(data => {
				...
				for (const result_key of Object.keys(data.query.results)) {
					...
					result = OSLRef.util.normalizeSmwMultilangResult(result, lang);
