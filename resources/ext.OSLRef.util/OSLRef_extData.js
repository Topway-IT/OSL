OSLRef.extData = class {
    ...
    static mapObjectMap(obj, map, data, mode) {
        for (const [k, v] of Object.entries(map)) {
            if (OSLRef.util.isString(v)) {
                var value = OSLRef.extData.getValue(data, v, mode);
                if (OSLRef.util.isArray(value)) value = OSLRef.util.uniqueArray(value);
                obj[k] = value;
            }
            else if (OSLRef.util.isObject(v)) {
                obj[k] = {};
                OSLRef.extData.mapObjectMap(obj[k], v, data, mode);
            }
    ...
    static mapTemplateMap(obj, map, data, mode) {
        for (const [k, v] of Object.entries(map)) {
            if (OSLRef.util.isString(v)) {
                const value = OSLRef.extData.getValue(data, k, mode);
                console.log("eval ", k, " with ", value);
                const template = Handlebars.compile(v);
                obj = OSLRef.util.mergeDeep(obj, JSON.parse(template(value)))
            }
            else console.log("invalide map entry: ", v)
        }
        return obj;
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            for (const data_source of data_source_maps) {
                ...
                if (data_source.request_object_map) {
                    options.body = {};
                    OSLRef.extData.mapObjectMap(options.body, data_source.request_object_map, jsondata, 'jsonpath');
            ...
            Promise.allSettled(promises).then((results) => {
                ...
                for (const result of results) {
                    ...
                    var ext_jsondata = {};
                    if (data_source.object_map) OSLRef.extData.mapObjectMap(ext_jsondata, data_source.object_map, data, data_source.mode);
                    if (data_source.template_map) ext_jsondata = OSLRef.extData.mapTemplateMap(ext_jsondata, data_source.template_map, data, data_source.mode);
                    jsondata = OSLRef.util.mergeDeep(jsondata, ext_jsondata);
                }
                resolve(jsondata);
            });
        });
    ...
    static async getAiCompletion(params) {
        ...
        let org_data_flatten = OSLRef.util.flatten(jsondata, "root", { notation: "dot", array_index_notation: ".0" });
        ...
        let data_flatten = OSLRef.util.flatten(data.result, "root", { notation: "dot", array_index_notation: ".0" });
        ...
        for (const [key, value] of Object.entries(data_flatten)) {
            ...
            if (subeditor.format === 'autocomplete') {
                ...
                let input = value
                jsondata = OSLRef.util.deepCopy(data.result);
        ...
        //console.log(data_flatten);
        jsondata = OSLRef.util.unflatten(data_flatten, { notation: "dot", array_index_notation: ".0" })["root"];
