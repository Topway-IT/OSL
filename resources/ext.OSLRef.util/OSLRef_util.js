OSLRef.util = class {
	...
	static OslId(uuid) {
		console.log("OslId is deprecated. Please use OSLRef.util.OswId");
		return OSLRef.util.OswId(uuid);
	}

	static OswId(uuid) {
		if (!uuid) uuid = OSLRef.util.uuidv4();
	...
	static toPascalCase(str) {
		str = OSLRef.util.replaceUmlaute(str);
		if (!OSLRef.util.isPascalCase(str)) {
			str = str.charAt(0).toUpperCase() + str.slice(1);
		}
		if (!OSLRef.util.isPascalCase(str)) {
			var camelCase = OSLRef.util.toCamelCase(str);
	...
	static toCamelCase(str) {
		str = OSLRef.util.replaceUmlaute(str);
		if (!OSLRef.util.isCamelCase(str)) {
	...
	static deepEqual(x, y) {
		...
		return x && y && tx === 'object' && tx === ty ? (
		  ok(x).length === ok(y).length &&
			ok(x).every(key => OSLRef.util.deepEqual(x[key], y[key]))
		) : (x === y);
	}
	...
	static uniqueArray(array) {
		...
		for (const item of array) {
			...
			for (const added_item of result) {
				if (OSLRef.util.deepEqual(added_item, item)) {
					add = false;
					continue;
				}
			}
	...
	static mergeDeep(target, source) {
		...
		if (OSLRef.util.isObject(target) && OSLRef.util.isObject(source)) {
			Object.keys(source).forEach(key => {
				if (OSLRef.util.isArray(source[key]) && OSLRef.util.isArray(target[key]) ) {
					if (!(key in target))
						Object.assign(output, { [key]: source[key] });
					else
						output[key] = OSLRef.util.uniqueArray(target[key].concat(...source[key]));
				} else if (OSLRef.util.isObject(source[key])) {
					if (!(key in target))
						Object.assign(output, { [key]: source[key] });
					else
						output[key] = OSLRef.util.mergeDeep(target[key], source[key]);
				} else {
					Object.assign(output, { [key]: source[key] });
				}
			});
			...
			if (jsonObj.hasOwnProperty(key)) {
				...
				if (Array.isArray(jsonObj[key])) {
					jsonObj[key].forEach((item, index) => {
						OSLRef.util.flatten(item, newKey + options.array_index_notation.replace("0", index), options, result);
					});
				} else if (typeof jsonObj[key] === 'object' && jsonObj[key] !== null) {
					OSLRef.util.flatten(jsonObj[key], newKey, options, result);
				} else {
	...
	static escapeDoubleQuotes(obj) {
		if (typeof obj === 'string') {
			...
			return obj.map(OSLRef.util.escapeDoubleQuotes);
		} else if (typeof obj === 'object' && obj !== null) {
			// Iterate over object properties
			const escapedObj = {};
			for (const key in obj) {
				if (obj.hasOwnProperty(key)) {
					escapedObj[key] = OSLRef.util.escapeDoubleQuotes(obj[key]);
				}
			}
			return escapedObj;
		}
	...
	static mergeJsonLdContextObjectList(context) {
		...
		*/
		...
		if (!OSLRef.util.isArray(context)) return context;
		
		var result = [];
		var last = null;
		for (let e of context) {
			if (OSLRef.util.isObject(e)) {
				if (!last) last = e;
				else last = OSLRef.util.mergeDeep(last, e);
			} else {
				if (last) {
					result.push(last);
					last = null;
	...
	static isDefined(arg) {
		return !OSLRef.util.isUndefined(arg);
	}

	static defaultArg(arg, default_value) {
		if (OSLRef.util.isDefined(arg)) return arg; //special case: boolean false
		else return default_value;
	}

	// returns the value of a table (dict) path or default, if the path is not defined
	static defaultArgPath(arg, path, default_value) {
		if (OSLRef.util.isUndefined(arg)) return default_value;
		else if (OSLRef.util.isUndefined(path)) return arg;
		else {
			var key = path.shift();
			if (OSLRef.util.isUndefined(key)) return arg;  //end of path
			return OSLRef.util.defaultArgPath(arg[key], path, default_value);
		}
	}
	...
	static getAbsolutePageUrl(title, params={}, pretty=true) {
		return mw.config.get("wgServer") + OSLRef.util.getRelativePageUrl(title, params, pretty);//mw.util.getUrl(title, params);
	}
