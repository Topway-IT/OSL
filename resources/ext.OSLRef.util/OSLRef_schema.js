...
OSLRef.schema = class {
    constructor(args) {
		...
		this.config = OSLRef.util.mergeDeep(defaultConfig, args.config);
        this.debug = OSLRef.util.defaultArg(args.debug, false);
        var jsonschema = args.jsonschema || {};

        if (OSLRef.util.isString(jsonschema)) jsonschema = JSON.parse(jsonschema);
    ...
    static selftest() {
        console.log("OSLRef.schema selftest");
        var jsonschema = {
            "@context": {
        ...
        var schema = new OSLRef.schema({jsonschema: jsonschema});
    ...
    getContext(config) {
        ...
        config = OSLRef.util.mergeDeep(_config, config);
        var res = {};
    ...
    _preprocess(params) {
        ...
        if (schema['$ref'] && OSLRef.util.isString(schema['$ref'])) schema['$ref'] = schema['$ref'].replaceAll("%24defs", "$defs")
        ...
        if (schema.type === "array" && schema.format === "table" && schema.items?.type === "string") {
            //schema.options = OSLRef.util.mergeDeep(schema.options, {"compact": true, "array_controls_top": false}); // display only item title
        ...
        if (schema['@context']) this._context = OSLRef.util.mergeDeep(schema['@context'], this._context); // merge nested context over general context
    ...
    getCategories(args = {}) {
        var jsonschema = OSLRef.util.defaultArg(this.getSchema(), {});
        var includeNamespace = OSLRef.util.defaultArg(args.includeNamespace, false);

        var categories = [];
        var allOf = jsonschema['allOf'];
        if (OSLRef.util.isDefined(allOf)) {
            if (!OSLRef.util.isArray(allOf)) allOf = [allOf]; // "allOf": {"$ref": "/wiki/Category:Test?action=raw"}
    ...
    _buildContext(args) {
        var schema = OSLRef.util.defaultArg(args.jsonschema, {});
        var context = OSLRef.util.defaultArg(schema['@context'], {});
        if (OSLRef.util.isObject(context)) return {context:context};
        var result = {};
        var index = 0;
        for (var subcontext of context) {
            if (OSLRef.util.isString(subcontext)){
                result["" + index] = subcontext;
                index++;
            } 
            else result = OSLRef.util.mergeDeep(result, subcontext)
        }
    ...
    _getSemanticProperties(args) {
        var jsondata = OSLRef.util.defaultArg(args.jsondata, {});
        var schema = OSLRef.util.defaultArg(args.schema, {});

        var debug = OSLRef.util.defaultArg(args.debug, true);
        ...
        if (schema.allOf) {
			if (Array.isArray(schema.allOf)) {
				for (const subschema of schema.allOf) {
					...
					//OSLRef.util.mergeDeep(this._getSemanticProperties(args), { properties: properties, definitions: property_data });
        ...
        if (OSLRef.util.isDefined(context)) {

            var schema_properties = OSLRef.util.defaultArg(schema.properties, {});
            if (debug) {
                for (const [k, v] of Object.entries(context)) {
                    this.log("" + k + " maps to " + v);
                    if (OSLRef.util.isNumber(k)) this.log("imports " + v)
                    else if (OSLRef.util.isObject(v)) this.log("" + k + " maps to " + v["@id"]) 
                    else this.log("" + k + " maps to " + v)
                }
            }
            for (const [k, v] of Object.entries(jsondata)) {
                if (OSLRef.util.isDefined(context[k])) {
                    if (debug) { this.log(context[k]) };

                    var property_definition = [];
                    if (OSLRef.util.isObject(context[k])) property_definition = context[k]["@id"].split(':');
                    ...
                    if (property_definition[0] === 'Property') {
                        ...
                        var schema_property = OSLRef.util.defaultArg(schema_properties[k], {});
    ...
    getPropertyDefinition(property_name) {
        ...
        if (OSLRef.util.isArray(res) && res.length === 1) res = res[0]; //unwrap array - wrap option may not work as expected: https://github.com/JSONPath-Plus/JSONPath/issues/72
        return res;
    }
    ...
    getSemanticQuery(args) {
        ...
        for (const [k, def] of Object.entries(semantic_properties.definitions)) {
            ...
            var filter = OSLRef.util.defaultArgPath(def.schema_data, ['options', 'role', 'query', 'filter'], 'eq');
            var value = def.value;
            ...
            if (OSLRef.util.isDefined(def.schema_data.title)) { select = select + "=" + def.schema_data.title }
        }
    ...
    static getAutocompleteQuery(subschema) {
        ...
        if (subschema.options?.autocomplete?.query) {
            ...
            (subschema.range && OSLRef.util.isString(subschema.range))
            || (subschema.subclassof_range && OSLRef.util.isString(subschema.subclassof_range))
