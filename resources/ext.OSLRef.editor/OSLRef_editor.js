...
OSLRef.editor = class {
	constructor(config) {
		var defaultConfig = {
			...
			id: 'json-editor-' + OSLRef.util.getShortUid(),
			...
			getSubjectId: (params) => { //callback to determine the currently edited subjects @id
				...
				//params.editor : The OSLRef editor instance
				return params.editor.config.target_namespace + ":" + OSLRef.util.OswId(params.jsondata.uuid);
			}
		};
		...
		this.config = OSLRef.util.mergeDeep(defaultConfig, config);
		this.flags = {
		...
		this.jsonschema = new OSLRef.schema({jsonschema: this.config.schema, config: {mode: this.config.mode, lang: this.config.lang, format: this.config.format}, debug: true});
	...
	_onsubmit(params = {}) {
		console.log('OSLRef: Using OSL _onsubmit method');
				...
				if (errors.length || validation_errors.length) {
					...
					if (this.config.allow_submit_with_errors) {
						msg += "<br>" + mw.message("mwjson-editor-save-anyway").text();
						        OSLRef.editor.prototype.confirm(msg).then((confirmed) => {
							if (confirmed) {
						...
						msg += "<br><br>" + mw.message("mwjson-editor-fix-all-errors").text();
						        OSLRef.editor.prototype.alert(msg).then(() => {
							reject();
						});
					}
			}
